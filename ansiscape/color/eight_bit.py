...
def get_8_bit_color(attr: int) -> Union[RGB, StandardColor]:
    if 0 <= attr <= 15:
        return get_8_bit_named_color(attr)
    return get_8_bit_rgb(attr)


def get_8_bit_named_color(attr: int) -> StandardColor:
    """
    Gets the named colour of the given attribute.

    The given attribute is typically the third in an 8-bit colour sequence. For
    example, given the sequence `"38;5;0"`, pass in `0`.
    """
    return StandardColor(attr)


def get_8_bit_rgb(attr: int) -> RGB:
    if 16 <= attr <= 231:
        return get_8_bit_rgb_color(attr)
    return get_8_bit_rgb_grey(attr)
...
def get_8_bit_rgb_visualization_html() -> str:
    css = (
...
def visualize_8_bit_rgb() -> None:
    ...
    with NamedTemporaryFile() as t:
        ...
        with open(path, "w") as f:
            f.write(get_8_bit_rgb_visualization_html())
